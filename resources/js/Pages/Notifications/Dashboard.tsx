...
import route from "@/ziggy"; // Use the default export from ziggy.js
...
export default function Dashboard({ auth }) {
  ...
  const handleSendReminders = () => {
    if (confirm("Are you sure you want to send exam reminders?")) {
      setIsLoading(true);
      window.axios.post(route("notifications.send"))
        .then(() => {
          setIsLoading(false);
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                          {upcomingExams.map((exam) => (
                            <tr key={exam.id}>
                              ...
                              <td className="px-6 py-4 whitespace-nowrap">
                                <div className="font-medium text-gray-900">{exam.start_time} - {exam.end_time}</div>
                                <div className="text-sm text-gray-500">{exam.venue}</div>
                              </td>
                              <td className="px-6 py-4 whitespace-nowrap">
                                <a 
                                  href={route('notifications.preview', exam.id)}
                          ...
                          <tbody className="bg-white divide-y divide-gray-200">
                            {recentNotifications.map((log) => (
                              <tr key={log.id}>
                                ...
                                <td className="px-6 py-4 whitespace-nowrap">
                                  <div className="font-medium text-gray-900">{log.notification_type.replace('App\\Notifications\\', '')}</div>
